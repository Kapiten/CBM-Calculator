...
public class ASelf {
    ...
    public static SharedPreferences getCalcHistory(Context c) {
        return c.getSharedPreferences("CalcPrefsFile_alpha", Context.MODE_PRIVATE);
    }
    
    public static SharedPreferences getCalcSet(Context c) {
        SharedPreferences sp = c.getSharedPreferences("CBMCalcSettings_alpha", Context.MODE_PRIVATE);
    ...
    public void onSave(Context c, int a) {
        try {
            if(arr.size()>0) {
                SharedPreferences.Editor ed = getCalcHistory(c).edit();
                
                ed.putString("jsonCalc"+a, Ut.toJSON(arr));
                ed.putInt("hc", a);
    ...
    public void setEntryNumberColors(Context c, int bgColor, int tColor) {
        ...
        LinearLayout llEntry = ((MainSimpleActivity)c).llEntry;
    ...
    public void setSymbolColors(Context c, ShapeDrawable sdColor, int tColor) {
        LinearLayout llEntry = ((MainSimpleActivity)c).llEntry;
        LinearLayout llToolbar = ((MainSimpleActivity)c).llToolbar;
    ...
    public void	entry(Context c, String str) {
        entry(c, str, new Object[]{((MainSimpleActivity)c).tvCurrent, ((MainSimpleActivity)c).llNC, ((MainSimpleActivity)c).btnPercentage, ((MainSimpleActivity)c).btnSignChange, ((MainSimpleActivity)c).hsvNC});
    }
    ...
    public void indexify(Context c) {
        for(int x = 0; x < ((MainSimpleActivity)c).llNC.getChildCount(); x++) {
            ((MainSimpleActivity)c).llNC.getChildAt(x).setTag(x);
        }
    }
    ...
    public void arrAct(Context c) {
        ...
        //FA=mDF.format(Double.parseDouble(ans+""));
                            ...
                            //}
            ((MainSimpleActivity)c).tvDoneCalculation.setText(answ2 + " | Write an expression");
        ...
        try{
            ...
            for(int b = 0; b<sz; b++) {
                    ...
                    for(int x = 0; x<l.size(); x++) {
                        ...
                        try{
                            if(!str.isEmpty()){
                    CurrentComponents.addCalculatedTV((MainSimpleActivity)c,
                                new JSONObject()
                    ...
                    sph.edit().apply();
                           ...
                           ex.printStackTrace();
                              Toast.makeText((MainSimpleActivity)c, ex.getMessage(), Toast.LENGTH_SHORT).show();
                            }
    ...
    public void setHCB(Context c, boolean z) {
        hcb=z;
        if(((Activity)c).equals(((MainSimpleActivity)c))) {
            ((Button)((MainSimpleActivity)c).llScifra.getChildAt(0)).setSelected(z);
            if(z) {

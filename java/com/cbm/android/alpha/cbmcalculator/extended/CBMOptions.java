...
public class CBMOptions {
    public static void funHistory(Context c, Object[] os) {
        try{
            ASelf aSelf = ASelf.get(c);
            if(ASelf.getCalcHistory(c).getAll().size()>0) {
               aSelf.setHCB(c, !aSelf.hcb);
               if(aSelf.isStandBy()) {
                   if(((Activity)c).equals((MainSimpleActivity)c))((MainSimpleActivity)c).clearC();
    ...
    public static void funPie(Context c, Object[] os) {
                    ASelf aSelf = ASelf.get(c);
    ...
    public static void funSignChange(Context c, Object[] os) {
        ASelf aSelf = ASelf.get(c);
